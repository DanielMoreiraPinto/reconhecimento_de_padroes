...
class DataLoaderVal_deblur(Dataset):
    def __init__(self, rgb_dir, img_options=None, rgb_dir2=None, imagePath=None):
        super(DataLoaderVal_deblur, self).__init__()


        if imagePath is None:
            inp_files = sorted(os.listdir(os.path.join(rgb_dir, 'test\\input')))
            tar_files = sorted(os.listdir(os.path.join(rgb_dir, 'test\\groundtruth')))
            self.inp_filenames = [os.path.join(rgb_dir, 'test\\input', x)  for x in inp_files if is_png_file(x)]
            self.tar_filenames = [os.path.join(rgb_dir, 'test\\groundtruth', x) for x in tar_files if is_png_file(x)]
        else:
            self.imagePath = imagePath
            inp_files = [os.path.basename(imagePath)]
            tar_files = [os.path.basename(imagePath)]
            self.inp_filenames = [imagePath]
            self.tar_filenames = [imagePath]
    ...
    def __getitem__(self, index):
        ...
        #inp_imgs = split.dividir_imagem(inp_img)
...
#        tar_img = Image.open(tar_path)
        ...
        inp_imgs = self.split_image(inp_img)
        imgs = []
        for inp_img in inp_imgs:
        # Validate on center crop
            if self.ps is not None:
                inp_img = TF.center_crop(inp_img, (ps,ps))
#                tar_img = TF.center_crop(tar_img, (ps,ps))
